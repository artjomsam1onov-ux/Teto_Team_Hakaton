...
OUTPUT_FILE = "Excel_результат.xlsx"
SUMMARY_FILE = "Итог.txt"
...
"""
    ...
    # Сначала ищем явные реальные проблемы.
    # Это важно: обращение может начинаться с "подскажите",
    # но дальше быть про воду, отопление, дорогу или аварию.
    for keyword in problem_keywords:
        if keyword in text_lower:
            return "проблема"

    # Потом отсекаем справочные вопросы и благодарности.
...
def main():
    ...
    with open(SUMMARY_FILE, "w", encoding="utf-8") as f:
        ...
        for _, row in summary_by_municipality.head(10).iterrows():
            f.write(
                f"- {row[municipality_col]} — {row['Количество проблем']} обращ.\n"
            )

        f.write("\nТоп проблем по муниципалитетам и темам:\n")
        for _, row in summary_by_topic.head(10).iterrows():
            f.write(
                f"- {row[municipality_col]}: {row[group_col]} / {row[category_col]} — {row['Количество']} обращ.\n"
            )
